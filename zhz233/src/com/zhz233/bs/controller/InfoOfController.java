...
public class InfoOfController {
	...
	private VGoodsMoreModel vGoodsMoreModel;
	...
	* @throws
	...
	public Map<String,Object> showAutoInfo(HttpServletRequest hsRequest,
		...
		vGoodsMoreModel = new VGoodsMoreModel();
		tGoodsRentModel.setGoods_no(GoodsNo);
		vGoodsMoreModel = tGoodsRentServiceImpl.selectTByNo(tGoodsRentModel);
		
		reMap.put("showInfo", vGoodsMoreModel);
	...
	* @throws
	...
	public Map<String, Object> showSearchInfo(HttpServletRequest hsRequest,
		...
		infoResult = new InfoResult();
		//vGoodsModelList;
		//System.err.println(searchModel.getSearch_type());
		//System.err.println(searchModel.getSearch_game());
		//System.err.println(searchModel.getSearch_server());
		
		if(searchModel.getSearch_price_low() < 0 || searchModel.getSearch_price_high() > 9999) {
			searchModel.setSearch_price_low(0);
			searchModel.setSearch_price_low(9999);
		}
		/*if(searchModel.getSearch_key() == null) {
			searchModel.setSearch_key(null);
		}*/
		if(searchModel.getSearch_auth()<=0) {
			searchModel.setSearch_auth(null);
		}
		if(searchModel.getSortStyle() < 0 || searchModel.getSortStyle() > 2) {
			searchModel.setSortStyle(0);
		}
		//searchModel.getShowStyle();
		if(searchModel.getStartPage()>0) {
			int tem = searchModel.getStartPage() - 1;
			searchModel.setStartPage(tem);
		}else {
			searchModel.setStartPage(0);
		}
		searchModel.setPageSize(8);
		//获取数据List
		vGoodsModelList = tGoodsServiceImpl.selectShowListT(searchModel);
		reMap.put("goodsList", vGoodsModelList);
		int StartPage = 0;
		if(searchModel.getStartPage()>0) {
			int tem = searchModel.getStartPage() + 1;
			StartPage = tem;
		...
		int TotalCount = vGoodsModelList.size();
		...
		reMap.put("TotalPage", TotalPage);
		//判断数据是否获取成功
		if(reMap != null) {//放置返回数据
			infoResult.setReMap(reMap);
			infoResult.setCode(ZHZConfig.REQCODE_1024);
			infoResult.setMsg("商品单信息获取成功");
			zhzResult.setCode(ZHZConfig.REQCODE_1022);
			zhzResult.setMsg("商品单信息获取成功");
		}else {
			infoResult.setCode(ZHZConfig.REQCODE_1023);
			infoResult.setMsg("商品单信息获取失败");
			zhzResult.setCode(ZHZConfig.REQCODE_1021);
			zhzResult.setMsg("商品单信息获取失败");
		}
		infoResult.setZtime(TimeUtils.getDate());
		zhzResult.setZtime(TimeUtils.getDate());
		map.put("iResult", infoResult);
